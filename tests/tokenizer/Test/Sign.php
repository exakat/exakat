<?php

namespace Test;

include_once(dirname(dirname(dirname(__DIR__))).'/library/Autoload.php');
spl_autoload_register('Autoload::autoload_test');
spl_autoload_register('Autoload::autoload_phpunit');

class Sign extends Tokenizer {
    /* 59 methods */
    public function testSign01()  { $this->generic_test('Sign.01'); }
    public function testSign02()  { $this->generic_test('Sign.02'); }
    public function testSign03()  { $this->generic_test('Sign.03'); }
    public function testSign04()  { $this->generic_test('Sign.04'); }
    public function testSign05()  { $this->generic_test('Sign.05'); }
    public function testSign06()  { $this->generic_test('Sign.06'); }
    public function testSign07()  { $this->generic_test('Sign.07'); }
    public function testSign08()  { $this->generic_test('Sign.08'); }
    public function testSign09()  { $this->generic_test('Sign.09'); }
    public function testSign10()  { $this->generic_test('Sign.10'); }
    public function testSign11()  { $this->generic_test('Sign.11'); }
    public function testSign12()  { $this->generic_test('Sign.12'); }
    public function testSign13()  { $this->generic_test('Sign.13'); }
    public function testSign14()  { $this->generic_test('Sign.14'); }
    public function testSign15()  { $this->generic_test('Sign.15'); }
    public function testSign16()  { $this->generic_test('Sign.16'); }
    public function testSign17()  { $this->generic_test('Sign.17'); }
    public function testSign18()  { $this->generic_test('Sign.18'); }
    public function testSign19()  { $this->generic_test('Sign.19'); }
    public function testSign20()  { $this->generic_test('Sign.20'); }
    public function testSign21()  { $this->generic_test('Sign.21'); }
    public function testSign22()  { $this->generic_test('Sign.22'); }
    public function testSign23()  { $this->generic_test('Sign.23'); }
    public function testSign24()  { $this->generic_test('Sign.24'); }
    public function testSign25()  { $this->generic_test('Sign.25'); }
    public function testSign26()  { $this->generic_test('Sign.26'); }
    public function testSign27()  { $this->generic_test('Sign.27'); }
    public function testSign28()  { $this->generic_test('Sign.28'); }
    public function testSign29()  { $this->generic_test('Sign.29'); }
    public function testSign30()  { $this->generic_test('Sign.30'); }
    public function testSign31()  { $this->generic_test('Sign.31'); }
    public function testSign32()  { $this->generic_test('Sign.32'); }
    public function testSign33()  { $this->generic_test('Sign.33'); }
    public function testSign34()  { $this->generic_test('Sign.34'); }
    public function testSign35()  { $this->generic_test('Sign.35'); }
    public function testSign36()  { $this->generic_test('Sign.36'); }
    public function testSign37()  { $this->generic_test('Sign.37'); }
    public function testSign38()  { $this->generic_test('Sign.38'); }
    public function testSign39()  { $this->generic_test('Sign.39'); }
    public function testSign40()  { $this->generic_test('Sign.40'); }
    public function testSign41()  { $this->generic_test('Sign.41'); }
    public function testSign42()  { $this->generic_test('Sign.42'); }
    public function testSign43()  { $this->generic_test('Sign.43'); }
    public function testSign44()  { $this->generic_test('Sign.44'); }
    public function testSign45()  { $this->generic_test('Sign.45'); }
    public function testSign46()  { $this->generic_test('Sign.46'); }
    public function testSign47()  { $this->generic_test('Sign.47'); }
    public function testSign48()  { $this->generic_test('Sign.48'); }
    public function testSign49()  { $this->generic_test('Sign.49'); }
    public function testSign50()  { $this->generic_test('Sign.50'); }
    public function testSign51()  { $this->generic_test('Sign.51'); }
    public function testSign52()  { $this->generic_test('Sign.52'); }
    public function testSign53()  { $this->generic_test('Sign.53'); }
    public function testSign54()  { $this->generic_test('Sign.54'); }
    public function testSign55()  { $this->generic_test('Sign.55'); }
    public function testSign56()  { $this->generic_test('Sign.56'); }
    public function testSign57()  { $this->generic_test('Sign.57'); }
    public function testSign58()  { $this->generic_test('Sign.58'); }
    public function testSign59()  { $this->generic_test('Sign.59'); }
}
?>