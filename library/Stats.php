...
class Stats {
    ...
    public function collect() {
        $this->stats['tokens_count']    = $this->queryOne("g.V.except([g.v(0)]){$this->file_filter}.hasNot('atom', 'Index').count()");
        ...
        $this->stats['isrm_array']      = $this->queryOne("g.V.has('atom', 'Array').filter{ it.in('ANALYZED').has('code', 'Analyzer\\\\Arrays\\\\IsRead').any() == false}.filter{ it.in('ANALYZED').has('code', 'Analyzer\\\\Arrays\\\\IsModified').any() == false}.count()");
