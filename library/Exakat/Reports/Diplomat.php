...
$html = <<<HTML
        ...
        <section class="content" id="dashboard">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
                <div class="box-body chart-responsive">
                  <div id="donut-chart_issues" style="max-height: 300px"></div>
                </div>
            </div>
...
HTML;
        ...
        $html = $this->makeAuditDate($html);

        $baseHTML = $this->getBasedPage('empty');
        $finalHTML = $this->injectBloc($baseHTML, 'TITLE', 'Issues\' dashboard');
        $finalHTML = $this->injectBloc($finalHTML, 'DESCRIPTION', '');

        // bloc Issues
        $issues = $this->getIssuesBreakdown();
        $html = $this->injectBloc($html, 'BLOCHASHDATA', $issues['html']);
        $html = $this->injectBloc($html, 'BLOCISSUES', $issues['html']);
        $tags[] = 'SCRIPTISSUES';
        $code[] = $issues['script'];
        ...
        $finalHTML = $this->injectBloc($finalHTML, 'BLOC-JS',  $this->getJS($tags, $code));
        $finalHTML = $this->injectBloc($finalHTML, 'CONTENT', $html);
...
JAVASCRIPT;
    ...
    private function getJS($tag, $code) {
    
        $id = array_search('SCRIPTISSUES', $tag);
        $issues = $this->getDonutJS('issues', $code[$id]);

        $id = array_search('SCRIPTSEVERITY', $tag);
        $severities = $this->getDonutJS('severity', $code[$id]);
...
JAVASCRIPT;
        ...
        return str_replace($tag, $code, $blocjs);
    }
...
JAVASCRIPT;
    ...
    public function getIssuesBreakdown() {
        ...
        $rank = 0;
        $names = array();
        foreach (array_slice($data, 0, 9) as $key => $value) {
            ++$rank;

            $names[$key] = $key;
            $issuesHtml .= '<div class="clearfix">
                   <div class="block-cell">'.$rank.') <a href="issues.html#analyzer='.$key.'" title="'.$names[$key].'">'.$names[$key].'</a></div>
                   <div class="block-cell text-center">'.$value.'</div>
                 </div>';
            $dataScript[] = '{label: "'.$key.'", value: '.( (int) $value).'}';
        }
        
        $others = array_slice($data, 10, 1000);
        $value = array_sum($others);
        $c = count($others);
        ++$rank;
        $issuesHtml .= '<div class="clearfix">
                   <div class="block-cell">'.$rank.') <a href="issues.html" title="Others ('.$c.')">Others ('.$c.')</a></div>
                   <div class="block-cell text-center">'.$value.'</div>
                 </div>';
        $others = 
        $dataScript[] = '{label: "Others", value: '.( $c).'}';
